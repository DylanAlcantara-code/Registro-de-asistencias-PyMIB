...
function generateQR() {
  ...
  if (!supervisor) {
    showToast('âš  Ingresa el nombre del supervisor', 'warning');
    document.getElementById('sup-name').focus();
    return;
  }
  if (!proyecto) {
    showToast('âš  Ingresa el proyecto/obra', 'warning');
  ...
  // â”€â”€ Render QR using QRCode constructor (qrcodejs API) â”€â”€
  ...
  startQRTimer();
  showToast('âœ“ QR generado exitosamente', 'success');
}
...
function startQRTimer() {
  ...
  function tick() {
    ...
    if (remaining <= 0) {
      ...
      showToast('âš¡ QR expirado â€” regenerando automÃ¡ticamente', 'info');
